...
/** PATCH [username]/jobs/[jobId]
 * updates a user to apply for a specific job
 */
router.patch('/:username/jobs/:jobId', ensureUserOrAdmin, async (req, res, next) => {
	try {
		const { username, jobId } = req.params;
		await User.apply(username, jobId);
		return res.json({ msg: 'job application added!' });
	} catch (err) {
		return next(err);
	}
});
